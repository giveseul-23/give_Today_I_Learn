...
public class Ex01_Hello extends HttpServlet {
    ...
    public Ex01_Hello() {
    	/*
    	  ...
    	  한 번 로딩되면 재실행되지 않는다
    	  두번째 URL 요청부터는 service() 메소드만 실행됨
    	 * */
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//request : 요청관련 정보(클라이언트에서 보내온 정보, 데이터)
		//response : 응답관련 정보(요청에 대한 결과를 전달할 정보, 데이터)
		System.out.println("> doGet() 실행");
		response.getWriter().append("Served at: ").append(request.getContextPath()); //단순 텍스트
		response.getWriter().println("<h1>hello</h1>"); //h1태그
	}
